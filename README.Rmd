...
## Details
The package allows simulation of convolution-closed count time series models with the \code{cocoSim}
function. Model fitting is performed with the \code{cocoReg} routine. By passing a cocoReg-type object,
\code{cocoForecast} computes the one-step ahead forecasting distribution. \code{cocoBoot}, \code{cocoPit},
\code{cocoScore}, and \code{cocoResid} provide routines for model assessment.
